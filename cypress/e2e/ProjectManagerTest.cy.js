//should login successfully with valid credentials for Project Manager with remaining agents
//should return a 404 status code for an invalid API endpoint
//should login successfully with valid credentials for a Project Manager
//should return an error when login attempt is made with an incorrect password
//should return 404 status when project_id is missing in the URL
//should return 400 Bad Request when an invalid project_id format (Non-Numeric) is provided
//should return 405 Method Not Allowed when POST method is used for GET endpoint
//should return a 405 Method Not Allowed when GET request is sent with a body
//should return 400 Bad Request when path parameter is given as a placeholder
//should return 401 Unauthorized when login fails and protected resource is accessed
//should return an error when project_id path parameter exceeds maximum length

//should return 404 Not Found when an invalid project_id is provided
//should return 404 Not Found for an invalid project ID
//should return 404 Not Found for invalid project path
//should return a status code based on the malformed URL
...
describe('Project Manager API Test Cases', () => {
    ...
    it('should return a 404 status code for an invalid API endpoint', () => {
        cy.request({
            ...
            expect(response.status).to.eq(404);        // but getting 200 OK
    ...
    it('should return 400 Bad Request when an invalid project_id format (Non-Numeric) is provided', () => {
        cy.request({
            ...
            // Check the status code
            expect(response.status).to.eq(400);        // but getiing 200 OK
            // Validate the response body
            expect(response.body.status).to.eq('success');
            expect(response.body.message).to.eq('List of Users not in Project');
            expect(response.body.data[0].user_id).to.eq(307);
...
it('should return an error when project_id path parameter exceeds maximum length', () => {
    cy.request({
        method: 'GET',   
        url: 'https://demo.atforte.com/api/remainingagents/12345656789023456ytgh3456789iuhgfdftyghjnm@#$%3245678ytgfdgcvbnm',
        headers: {
            'Content-Type': 'application/json',
        },
        failOnStatusCode: false,
    }).then((response) => {
        // Expected error status (Bad Request)
        expect(response.status).to.eq(400);      // but getting 200 OK
    })
})
...
// ++++++++++++++++++  Project Agents ++++++++++++++++ //
   ...
   it('should return 404 Not Found when an invalid project_id is provided', () => {
    cy.request({
        method: 'GET',   
        url: 'https://demo.atforte.com/api/projectagents/{project_id}',
        headers: {
            'Content-Type': 'application/json',
        },
        failOnStatusCode: false,
    }).then((response) => {
        // Expected error status (404 Not Found)
        expect(response.status).to.eq(404);      // but getting 200 OK
    })
  })

  it('should return 404 Not Found for an invalid project ID', () => {
    cy.request({
        method: 'GET',   
        url: 'https://demo.atforte.com/api/projectagents/{1}',
        headers: {
            'Content-Type': 'application/json',
        },
        failOnStatusCode: false,
...
it('should return 404 Not Found for invalid project path', () => {
    cy.request({
        method: 'GET',   
        url: 'https://demo.atforte.com/api/projectagents/{1}=6787',
        headers: {
            'Content-Type': 'application/json',
        },
        failOnStatusCode: false,
...
it('should return a status code based on the malformed URL', () => {
    cy.request({
        method: 'GET',   
        url: 'https://demo.atforte.com/api/projectagents/186=-%098765?email=testuser@example.com&password=TestPassword123',
        headers: {
            'Content-Type': 'application/json',
        },
        failOnStatusCode: false,
    }).then((response) => {
        expect(response.status).to.be.oneOf([404, 400, 500]);  // but getting 200 OK
})
})
